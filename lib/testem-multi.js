  ...
  exports.exec = function(config, callback){
    ...
    async.reduce(
      ...
      function(memo, path, callback){
        ...
        testem.config.read(function(){
          testem.app.on('all-test-results', function(){
            var results = [];
            testem.app.tapProducer.on('data', function( data ){
              results.push( data );
            });
            testem.app.tapProducer.on('end', function(){
              var result = _.chain(results.join('').split('\n')),
                tests = memo.tests,
